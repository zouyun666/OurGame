...
Item {
    ...
    signal juicyMeterFull()//果汁满的信号
    signal juicyMeterEmpty()//果汁为空的信号
    ...
    //自定义一个属性，会隐式地为该属性创建一个值改变信号，以及一个相应的信号处理器
    onPercentageChanged: {
        if(percentage <= 0)
            juicyMeterEmpty()
    ...
    SequentialAnimation{
        ...
        //此属性保存动画应播放的次数。默认情况下，循环为1：动画将播放一次然后停止。如果设置为Animation.Infinite，动画将不断重复，直到它被显式停止 - 通过将running属性设置为false，或者通过调用stop（）方法。
